...
// Get current guru's ID
$current_user_id = $_SESSION['user_id'];
$current_guru_query = "SELECT id_guru FROM guru WHERE id_user = $1";
$current_guru_result = pg_query_params($conn, $current_guru_query, array($current_user_id));
$current_guru = pg_fetch_assoc($current_guru_result);

if (!$current_guru) {
    echo "Error: Guru data not found.";
    exit();
}

$current_guru_id = $current_guru['id_guru'];

// Get guru data for dropdown (only current guru for guru role)
...
$guru_result = pg_query_params($conn, $guru_query, array($current_guru_id));
...
if (isset($_GET['msg'])) {
    if ($_GET['msg'] === 'success') {
        ...
        $message = '<div class="alert alert-danger">
            ...
            Anda sudah mengajar <strong>' . htmlspecialchars($jenis) . '</strong> 
            di ' . htmlspecialchars($kelas) . ' pada tanggal ' . htmlspecialchars($tanggal) . ' 
            pada jam ' . htmlspecialchars($jam) . '.<br>
            <small class="text-muted">Anda tidak dapat mengajar di lebih dari 1 kelas pada waktu yang bersamaan. Silakan pilih tanggal lain atau waktu yang berbeda.</small>
        </div>';
    }
}
?>
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            ...
                            <form action="../../app/controllers/proses_tambah_kegiatan.php" method="POST" onsubmit="return validateForm()">
                                <!-- Hidden field to identify guru role -->
                                <input type="hidden" name="user_role" value="guru">
                                
                                <div class="form-group">
                                    <label for="id_guru">Guru</label>
                                    <select class="form-control" id="id_guru" name="id_guru" required readonly style="background-color: #f8f9fa;">
                                        <?php while ($guru = pg_fetch_assoc($guru_result)): ?>
                                            <option value="<?php echo $guru['id_guru']; ?>" selected>
                                                <?php echo htmlspecialchars($guru['nama_guru']); ?> (Anda)
                                            </option>
                                        <?php endwhile; ?>
                                    </select>
                                    <small class="form-text text-muted">Anda hanya dapat menambahkan kegiatan untuk diri sendiri</small>
                                </div>
                            ...
                            <div class="mt-4 p-3 bg-light rounded">
                                ...
                                <ul class="mb-0 text-sm">
                                    ...
                                    <li><strong>Anda tidak dapat mengajar di lebih dari 1 kelas</strong> pada waktu yang bersamaan</li>
